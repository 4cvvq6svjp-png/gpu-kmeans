...
void gpu_GetResultOnCPU()
{
  ...
  CHECK_CUDA_SUCCESS(cudaMemcpy(centroid, GPU_centroid_T, 
...
/*-------------------------------------------------------------------------------*/
__global__ void kernel_InitializeCentroids(curandState *state,
                                           T_real *GPU_centroid_T,
                                           T_real *GPU_instance_T)
{
  int centroidIdx = threadIdx.x + blockIdx.x * blockDim.x;
...
/*-------------------------------------------------------------------------------*/
__global__ void kernel_ComputeAssign(T_real *GPU_centroid_T,
                                     T_real *GPU_instance_T,
                                     int *GPU_label)
{
  ...
  if (idx < NB_INSTANCES) {
        ...
        if (old_label != best_cluster) {
            GPU_label[idx] = best_cluster;
            atomicAdd(&GPU_change_total, 1ULL); // condition d'arrêt
        }
    }
}
...
/*-------------------------------------------------------------------------------*/
/* Update centroids - step 1 - ACCUMULATION                                      */
/*-------------------------------------------------------------------------------*/
__global__ void kernel_UpdateCentroid_Step1(int *GPU_label,
                                            T_real *GPU_instance_T,
                                            T_real *GPU_centroid_T,
                                            int *GPU_count)
{
...
/*-------------------------------------------------------------------------------*/
/* Update centroids - step 2 - MOYENNE                                           */
/*-------------------------------------------------------------------------------*/
                                            ...
                                            int *GPU_count)
{
 ...
 if (idx < NB_CLUSTERS) {
  int count = GPU_count[idx]; // on récupère le nombre d'instance qui ont participer à la somme

  if (count > 0) {
    for (int d = 0; d < NB_DIMS; d++) {
      GPU_centroid_T[idx + d * NB_CLUSTERS] /= count; // On fait la moyenne, ça nous donne les nouvelles coordonnées du cluster
    }
  }
 }
 // Note:
...
void gpu_Kmeans()
{
  ...
  do {
    ...
    Dg.y = 1;
    Dg.z = 1;
    kernel_ComputeAssign<<<Dg,Db>>>(GPU_centroid_T,
                                                  GPU_instance_T,
                                                  GPU_label);
    CudaCheckError();
    ...
    CHECK_CUDA_SUCCESS(cudaMemset(GPU_centroid_T, 0, sizeof(T_real)*NB_DIMS*NB_CLUSTERS), "Reset GPU centroids");
    ...
    Dg.y = 1;
    Dg.z = 1;
    kernel_UpdateCentroid_Step1<<<Dg,Db>>>(GPU_label,
                                           GPU_instance_T,
                                           GPU_centroid_T,
                                           GPU_count);
    ...
    Dg.y = 1;
    Dg.z = 1;
    kernel_UpdateCentroid_Step2<<<Dg,Db>>>(GPU_centroid_T,
                                           GPU_count);
    CudaCheckError();
